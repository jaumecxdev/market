...
class ImportAlco
{
    ...
    const URI_PRODUCTS = 'https://alcodistribuciones.com/feed/articulos.php?m=feed';
    const URI_OFFERS = 'https://alcodistribuciones.com/feed/articulos.php?m=feed';
    ...
    static function products(Supplier $supplier, Collection $productsCollect)
    {
       try {
            ...
            foreach($productsCollect as $row) {
                ...
                $name = FacadesMpe::getString($row['H']);
                ...
                $supplierSku = $row['A'];
                [$pn, $ean] = FacadesMpe::getPnEan(null, $row['B']);
                $longdesc = FacadesMpe::getText($row['I']);
                $weight = $row['O'];      //FacadesMpe::roundFloatEsToEn($row['O']);
                ...
                // Create new product
                    ...
                    if (isset($row['G']) && $row['G'] != '')
                        $product->updateOrCreateExternalImage($row['G']);
                    else
    ...
    {
        try {
            return utf8_decode($row['Q'].' / '.$row['R']);
