...
@push('menu')
    <li class="nav-item d-none d-sm-inline-block">
        <a href="{{ route('shops.shop_params.sync', [$shop]) }}" class="btn btn-danger">IMPORTANTE: SINCRONIZAR TABLAS DESPUÉS DE MODIFICAR</a>
    </li>
    <li class="nav-item d-none d-sm-inline-block">
        <a href="{{ route('shops.shop_filters.index', [$shop]) }}" class="nav-link"><i class="fas fa-filter"></i> Filtros</a>
    </li>
    <li class="nav-item d-none d-sm-inline-block">
    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-10"><h1>AÑADIR PARÁMETRO PARA LA TIENDA: ({{ $shop->market->name }}) {{ $shop->name }}</h1></div>
                </div>
            </div>
        </section>
                ...
                <div class="col-12">
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            @if (isset($shop_param))
                                <form method="post" action="{{ route('shops.shop_params.update', [$shop, $shop_param]) }}">
                                    @method('PATCH')
                            @else
                                <form method="post" action="{{ route('shops.shop_params.store', [$shop]) }}">
                            @endif
                                @csrf
                                ...
                                <div class="form-group row">
                                    <label for="supplier_id" class="col-sm-2 col-form-label">Proveedor | SKU | Producto</label>
                                    <div class="col-sm-2">
                                        @include('forms.supplier', ['suppliers' => $suppliers, 'supplier_id' => $shop_param->supplier_id ?? old('supplier_id')])
                                    </div>

                                    <div class="col-sm-2">
                                        @include('forms.supplier_sku', ['supplierSku' => $shop_param->supplierSku ?? old('supplierSku')])
                                    </div>
                                    <div class="col-sm-6">
                                ...
                                <div class="form-group row">
                                    <label for="brand_id" class="col-sm-2 col-form-label">Marca | Root | categorías</label>
                                    <div class="col-sm-2">
                                        @include('forms.brand', ['brand_id' => $shop_param->brand_id ?? old('brand_id'), 'brand_name' => $shop_param->brand->name ?? old('brand_name')])
                                    </div>
                                    ...
                                    <div class="col-sm-3">
                                        @include('forms.category', ['category_id' => $shop_param->category_id ?? old('category_id'), 'category_name' => $shop_param->category->name ?? old('category_name')])
                                    </div>
                                </div>
                                ...
                                <div class="form-group row">
                                    <label for="fee" class="col-sm-2 col-form-label">% Rappel | € Ports</label>
                                    {{-- <div class="col-sm-2">
                                ...
                                <div class="form-group row">
                                    <label for="fee" class="col-sm-2 col-form-label">% Bcio | % Mpe</label>
                                    <div class="col-sm-2">
                                        @include('forms.fee', ['fee' => $shop_param->fee ?? old('fee')])
                                        <small>Margen de beneficio %.</small>
                                    </div>
                                    <div class="col-sm-2">
                                        @include('forms.mps_fee', ['mps_fee' => $shop_param->mps_fee ?? old('mps_fee')])
                                        <small>Intermediación Mpe %.</small>
                                    </div>
                                </div>

                                <div class="form-group row">
                                    <label for="fee" class="col-sm-2 col-form-label">€ Bcio Mín | % RePrice Mín</label>
                                    <div class="col-sm-2">
                                ...
                                <div class="panel-body">
                                    ...
                                    <table class="table table-striped">
                                        <tr class="table-warning">
                                            ...
                                            <th>Cliente<br>Mpe</th>
                                            <th>Mín<br>RePrice</th>
                                        ...
                                        @foreach($shop_params as $shop_param)
                                            <tr shop_param-id="{{ $shop_param->id }}">
                                                ...
                                                <td>{{ isset($shop_param->supplierSku) ? ('SKU: '.$shop_param->supplierSku) :
                                                    ...
                                                    '' )))))) }}</td>
                                                <td>{{ $shop_param->brand_id ? $shop_param->brand->name : '' }}</td>
                                                <td>{{ $shop_param->root_category_id ? '('.$shop_param->root_category->marketCategoryId.') '.$shop_param->root_category->name : '' }}
                                                    ...
                                                    {{ $shop_param->category_id ? '| '.$shop_param->category_id.' ('.$shop_param->category->code.') '.$shop_param->category->name : '' }}</td>
                                                ...
                                                <td>{{ $shop_param->canon != 0 ? $shop_param->canon.' €' : '' }}<br>
                                                    {{ $shop_param->rappel != 0 ? $shop_param->rappel.' %' : '' }}<br>
                                                    {{ $shop_param->ports != 0 ? $shop_param->ports.' €' : '' }}</td>
                                                <td>{{ $shop_param->fee != 0 ? $shop_param->fee.' %' : '' }}<br>
                                                    {{ $shop_param->mps_fee != 0 ? $shop_param->mps_fee.' %' : '' }}</td>
                                                <td>{{ $shop_param->bfit_min != 0 ? $shop_param->bfit_min.' €' : '' }}<br>
                                                ...
                                                <td class="row">
                                                    <a class="mr-2"
                                                        href="{{ route('shops.shop_params.edit', [$shop, $shop_param]) }}" data-toggle="tooltip" title="Editar">
                                                        <i class="far fa-edit"></i></a>
                                                    {{-- <form class="delete" action="{{ route('shops.shop_params.destroy', [$shop, $shop_param]) }}" method="post">
                                                        @method('delete')
                                                        @csrf
